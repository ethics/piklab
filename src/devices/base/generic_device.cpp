...
const Device::Package::TypeData Device::Package::TYPE_DATA[] = {
  ...
  { "qfn",   I18N_NOOP("QFN"),    Square, {  0, 16,  0, 20, 28, 44, 64,  0,   0 } },
  { "uqfn",  I18N_NOOP("UQFN"),   Square, {  0, 16,  0, 20, 28, 44,  0,  0,   0 } },
...
{
  double vdd = 0.0;
  for (uint i=0; i<uint(vdds.size()); i++) {
...
const Device::Package::TypeData *Device::Package::data(const QString &name)
{
  for (uint i=0; TYPE_DATA[i].name; i++)
    if ( name==TYPE_DATA[i].name ) return &TYPE_DATA[i];
  return 0;
}
...
{
  ASSERT( index<=size() && s<=int(size()-index) && s>=-1 );
...
//-----------------------------------------------------------------------------
const Device::VoltageType::Data Device::VoltageType::DATA[Nb_Types] = {
  { I18N_NOOP("Programmer Vpp") },
  { I18N_NOOP("Target Vdd")     },
  { I18N_NOOP("Target Vpp")     }
};

QString Device::VoltageValue::label() const
{
  if ( _type==Normal ) return i18n("%1 V").arg(formatDouble(_value, 1));
  return "---";
}

const Device::TargetMode::Data Device::TargetMode::DATA[Nb_Types] = {
  { I18N_NOOP("Stopped")        },
  { I18N_NOOP("Running")        },
  { I18N_NOOP("In Programming") }
};

const Device::ResetMode::Data Device::ResetMode::DATA[Nb_Types] = {
  { I18N_NOOP("Reset Held")     },
  { I18N_NOOP("Reset Released") }
};
...
{
  double vdd = 0.0;
  for (uint i=0; i<uint(_frequencyRanges.size()); i++) {
...
{
  s << package._names << package._pins;
  return s;
}
