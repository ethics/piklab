...
bool getMemoryRange(MemoryRangeType type, QDomElement element)
{
  ...
  if (type == MemoryRangeType::Cal) {
    ...
    if (data()->isCalibrationAtCodeEnd()) {
      ...
      if (data()->_calibration.opcodeMask != 0) qFatal("Calibration opmask should be 0");
...
void processName(const Pic::Config::Mask &cmask, BitValue pmask, Pic::Config::Value &cvalue)
{
  ...
  if ( cmask.value.isInside(pmask) ) { // protected bits
    ...
    const QStringList &ecnames = cvalue.extraConfigNames[Pic::ConfigNameType::Default];
...
virtual void processDevice(QDomElement device)
{
  ...
  if ( (data()->_architecture.data().properties & MaybeSelfWrite) && data()->_memoryTechnology==Device::MemoryTechnology::Flash ) {
    if ( sw=="yes" ) data()->_selfWrite = true;
    else if ( sw=="no" ) data()->_selfWrite = false;
    else qFatal("Malformed or missing self-write field");
  } else {
    if ( !sw.isEmpty() ) qFatal("Self-write is set for the whole family or non-flash device");
    data()->_selfWrite = (data()->_memoryTechnology!=Device::MemoryTechnology::Flash ? false : bool(data()->_architecture.data().properties & SelfWrite));
  }
  ...
  FOR_EACH(ProgVoltageType, vtype) {
    ...
    if ( !getVoltages(vtype, device) ) {
      switch (vtype.type()) {
        ...
        case ProgVoltageType::Nb_Types: ASSERT(false); break;
      }
    }
  }
...
void processMirrored(QDomElement element)
{
  ...
  if ( !mirrored.isEmpty() ) static_cast<RegistersData *>(data()->_registersData.get())->mirrored.append(mirrored);
}
...
void processUnused(QDomElement e)
{
  ...
  static_cast<RegistersData *>(data()->_registersData.get())->unused.append(rd);
}
...
void processSfr(QDomElement e)
{
  ...
  static_cast<RegistersData *>(data()->_registersData.get())->sfrs[name] = rdata;
}
...
void processCombined(QDomElement e)
{
  ...
  static_cast<RegistersData *>(data()->_registersData.get())->combined[name] = rdata;
}
...
void processDeviceRegisters(QDomElement element)
{
  ...
  if ( !s.isEmpty() ) {
    ...
    const RegistersData& rd = *static_cast<const RegistersData*>(d->_registersData.get());
    data()->_registersData.reset(new RegistersData(rd));
    return;
  }

  RegistersData &rdata = *static_cast<RegistersData*>(data()->_registersData.get());
  bool ok = false;
...
void processRegistersFile(const QString &filename, QStringList &devices)
{
    ...
    if ( child.isComment() ) qDebug("comment: %s", child.toComment().data().latin1());
      ...
      if ( _map.contains(name) ) {
        ...
        if ( devices.contains(name) ) {
          warning(QString("Registers already defined for %1").arg(name));
          error = true;
        }
  ...
  _data = NULL;
  if (error) qFatal("There were errors");
}
...
virtual void checkPins(const QMap<QString, uint> &pinLabels) const
{
  ...
  for (QMap<QString, uint>::const_iterator it = pinLabels.begin(); it != pinLabels.end(); ++it) {
    if ( it.key()=="VDD" || it.key()=="VSS" || it.key().startsWith("CCP") || it.key()=="NC" ) continue;
