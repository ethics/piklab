...
Pic::MemoryEditorLegend::Data::Data(const QString &text, QWidget *parent)
{
  ...
  KAction *a = new KAction(KIcon("top"), i18n("Go to start"), parent);
  connect(a, SIGNAL(triggered()), parent, SLOT(gotoStart()));
  actions.append(a);
  button->addAction(a);
  a = new KAction(KIcon("bottom"), i18n("Go to end"), parent);
  connect(a, SIGNAL(triggered()), parent, SLOT(gotoEnd()));
  actions.append(a);
  button->addAction(a);
...
void Pic::MemoryUserIdEditor::init(bool first)
{
  ...
  _setToChecksum = new KToggleAction(i18n("Set to unprotected checksum"), this);
  connect(_setToChecksum, SIGNAL(triggered()), this, SLOT(toggleSetToChecksum()));
  ...
  if ( readConfigEntry(BaseGlobalConfig::UserIdSetToChecksum).toBool() && memory().isClear(MemoryRangeType::UserId) ) {
    _setToChecksum->activate(QAction::Trigger);
    toggleSetToChecksum();
  }
}
