  ...
  if ( rdata.nbBanks!=1 ) {
    if ( data.is18Family() ) {
      if ( (i/2)==0 ) {
        ...
        top->addMultiCellWidget(label, row,row, 0,6, Qt::AlignHCenter);
      } else {
        _bankCombo = new QComboBox(this);
        for (uint k=1; k<2*rdata.nbBanks-1; k++) {
          if ( !rdata.isBankUsed(k/2) ) continue;
          if ( (k%2)==0 ) {
            int offset = rdata.firstUsedRegisterOffset(k/2);
            if ( offset==-1 || offset>=int(rdata.nbRegistersPerBank()/2) ) continue;
            _bankCombo->insertItem(i18n("Bank %1 (low)").arg(k/2));
          } else {
            int offset = rdata.lastUsedRegisterOffset(k/2);
            if ( offset==-1 || offset<int(rdata.nbRegistersPerBank()/2) ) continue;
            _bankCombo->insertItem(i18n("Bank %1 (high)").arg(k/2));
          }
        }
        if ( _bindex==3 ) _bankCombo->setCurrentItem(1);
        connect(_bankCombo, SIGNAL(activated(int)), SLOT(bankChanged()));
        top->addMultiCellWidget(_bankCombo, row,row, 0,6, Qt::AlignHCenter);
      }
    } else {
...
void Pic::BankWidget::updateView()
{
  ...
  for (int i=0; i<_registers.count(); i++) {
	...
	BitValue value = Register::list().value(rtd);
        if ( value!=Register::list().oldValue(rtd) ) _registers[i].edit->setColor(Qt::red);
    ...
    _readAllButton(0), _clearAllButton(0)
{
  ...
  if ( debugging && nb!=0 ) {
    ...
    grid->addWidget(_clearAllButton, 0, 0);
