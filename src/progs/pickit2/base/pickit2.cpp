...
{
  const Pic::Data& device = static_cast<const Pic::Data&>(*Device::Lister::instance().data("18F2455"));
...
bool Pickit2::USBPort::writeFirmwareCodeMemory(const Device::Array &data, ProgressMonitor &monitor)
{
  ...
  for (uint i=0x1000; i<data.count(); i+=16) { // 2 bytes/word and 32 bytes/command max
    ...
    for (; end>start; end--) if ( data[i + end].isInitialized() ) break;
    uint nb = end - start + 1;
    ByteArray cmd = createCommand(2, 2*nb, 2*i);
    for (uint k=0; k<nb; k++) {
  ...
  ByteArray cmd = createCommand(2, 2, 0x7FFE);
  cmd[5] = 0x55;
  cmd[6] = 0x55;
  return command(cmd);
}
...
//-----------------------------------------------------------------------------
bool Pickit2::Hardware::readVoltages(Device::VoltageValues &voltages)
{
  ...
  voltages[Device::VoltageType::TargetVdd] = Device::VoltageValue(a[0] & 0x08 ? Device::VoltageValue::Error : Device::VoltageValue::Normal, double(a[1] + a[2]*256) * 5.0 / 65535);
  voltages[Device::VoltageType::TargetVpp] = Device::VoltageValue(a[0] & 0x10 ? Device::VoltageValue::Error : Device::VoltageValue::Normal, double(a[3] + a[4]*256) * (5.0 /65535) * (7.4 / 2.7));
  return true;
}
...
bool Pickit2::Hardware::setVddVpp(double vdd, double vpp)
{
  ...
  for (uint i=0; i<30; i++) {
    Port::usleep(50000);
    Device::VoltageValues voltages;
    if ( !readVoltages(voltages) ) return false;
    if ( voltages[Device::VoltageType::TargetVpp].value()<(vpp+0.5) ) break;
  }
  return true;
}
...
bool Pickit2::P18F::doEraseRange(Pic::MemoryRangeType type)
{
  ...
  ASSERT( type==Pic::MemoryRangeType::Code );
...
bool Pickit2::P18F::doRead(Pic::MemoryRangeType type, uint wordOffset, Device::Array &data, const ::Programmer::VerifyData *vdata)
{
  ASSERT( wordOffset==0 );
  ...
  switch (type.type()) {
    ...
    case Pic::MemoryRangeType::Config: {
      ByteArray cmd = port.createCommand(0x80, data.count(), device().range(type).start.toUInt());
      if ( !port.command(cmd) ) return false;
    ...
    case Pic::MemoryRangeType::Eeprom:
      for (uint i=0; i<data.count();) {
       ByteArray cmd = port.createCommand(0x81, 64, i, false);
        if ( !port.command(cmd) ) return false;
        if ( !hardware().port().receiveWords(1, 1, words) ) return false;
        for (uint k=0; k<64; k++) {
          data[i] = words[k];
...
bool Pickit2::P18F::doWrite(Pic::MemoryRangeType type, uint wordOffset, const Device::Array &data, bool force)
{
  ASSERT( wordOffset==0 );
  ASSERT( data.count()==device().nbWords(type) );
  Q_UNUSED(force);
  ...
  switch (type.type()) {
    case Pic::MemoryRangeType::UserId: {
      ByteArray cmd = port.createCommand(0x82, data.count()/2, device().range(type).start.toUInt());
    ...
    case Pic::MemoryRangeType::Code: {
      ...
      for (uint i=0; i<data.count();) {
        bool allBlank = true;
        ByteArray cmd = port.createCommand(0x82, nb, 2*i);
        for (uint k=0; k<nb; k++) {
    ...
    case Pic::MemoryRangeType::Eeprom:
      for (uint i=0; i<data.count();) {
        bool allBlank = true;
        ByteArray cmd = port.createCommand(0x87, 32, i, false);
        for (uint k=0; k<32; k++) {
