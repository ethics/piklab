...
inline ByteArray createArray() { return ByteArray(64, 'Z', PrintAlphaNum); }
...
public:
  ...
  void fillCommand(ByteArray &cmd, uchar command, uint nbBytes, uint address, uint i, bool longAddress = true) const;
  ByteArray createCommand(uchar cmd, uint nbBytes, uint address, bool longAddress = true) const;
...
public:
  ...
  Hardware& hardware() const { return static_cast<Hardware&>(Pickit::DeviceSpecific::hardware()); }
  virtual bool init();
