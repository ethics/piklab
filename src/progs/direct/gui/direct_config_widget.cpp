...
Direct::HConfigWidget::HConfigWidget(::Programmer::Base &base, QWidget *parent, bool edit)
  : ::Hardware::HConfigWidget(base, parent, edit), _type(Normal)
{
  // pins assignment
  QGroupBox *groupb = new Q3GroupBox(1, Qt::Horizontal, i18n("Pin assignment"), this);
  ...
  for (uint i=0; i<Nb_PinTypes; i++) {
    ...
    QToolTip::add(label, i18n(PIN_DATA[i].comment));
    grid->addWidget(label, i, 0);
    _combos[i] = new KComboBox(w);
    _combos[i]->setEnabled(edit);
    connect(_combos[i], SIGNAL(activated(int)), SLOT(slotPinChanged()));
    QToolTip::add(_combos[i], i18n(PIN_DATA[i].comment));
    ...
    if (edit) {
      ...
      QToolTip::add(_testcbs[i], i18n(PIN_DATA[i].testComment));
  ...
  if (edit) {
    _sendBitsButton = new KPushButton(i18n("Send 0xA55A"), this);
...
{
  ASSERT( _connected && ptype!=DataIn );
...
bool Direct::HConfigWidget::set(const Port::Description &pd, const ::Hardware::Data &data)
{
  ...
  for (uint i=0; i<Nb_PinTypes; i++) {
    ...
    _combos[i]->setCurrentIndex(hardware()->indexForPin(dir, hdata.data.pins[i]));
    _invcbs[i]->setChecked(hdata.data.pins[i]<0);
    updatePin(PinType(i));
  }
  _delay->setValue(hdata.data.clockDelay);
  _type = hdata.data.type;
