...
{
  Config config;
  uint speed = config.readUIntEntry("port_speed", 19200);
...
{
  Config config;
  config.writeEntry("port_speed", Port::Serial::SPEED_VALUES[speed]);
}

uint TinyBootloader::Config::readTimeout()
{
  Config config;
  return config.readUIntEntry("timeout", 300);
}
...
{
  Config config;
  return config.readUIntEntry("nb_retries", 5);
}
void TinyBootloader::Config::writeRetries(uint nb)
{
  Config config;
  config.writeEntry("nb_retries", nb);
}
...
bool TinyBootloader::Hardware::verifyDeviceId()
{
  ...
  QStringList list = _base.group().supportedDevices();
  QStringList devices;
  QStringList::const_iterator it;
  for (it=list.begin(); it!=list.end(); ++it)
    if ( _id==data(*it).id ) devices.append(*it);
  if ( _id!=id ) {
...
{
  if ( device().architecture()==Pic::Architecture::MidRange
       || device().architecture()==Pic::Architecture::EMidRange ) return false;
