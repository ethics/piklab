...
extern ByteArray createArray() { return ByteArray(64, EndOfBuffer, PrintEscapeAll); }
...
public:
  ...
  bool setTargetReset(Device::ResetMode mode);
  bool setVoltages(double vdd, const FamilyData &fdata);
  ...
  bool readMemory(Pic::MemoryRangeType type, uint wordOffset, Device::Array &data, const ::Programmer::VerifyData *vdata);
  bool writeMemory(Pic::MemoryRangeType type, uint wordOffset, const Device::Array &data, bool force);
  bool eraseAll();
...
private:
  ...
  bool prepareRead(Pic::MemoryRangeType type, uint wordOffset);
  bool downloadData(const QValueVector<uint> &data, uint &index, bool clearBuffer);
  bool prepareWrite(Pic::MemoryRangeType type, uint wordOffset);
};
...
public:
  ...
  virtual bool doRead(Pic::MemoryRangeType type, uint wordOffset, Device::Array &data, const ::Programmer::VerifyData *vdata) {
    return hardware().readMemory(type, wordOffset, data, vdata);
  }
  virtual bool doWrite(Pic::MemoryRangeType type, uint wordOffset, const Device::Array &data, bool force) {
    return hardware().writeMemory(type, wordOffset, data, force);
  }
};
