...
bool Icd1::Hardware::readVoltages(Device::VoltageValues &voltages)
{
  if ( !sendCommand(0x7000) ) return false;
  BitValue res;
  if ( !sendCommand(0x701C, &res) ) return false;
  double v = (2.050 / 256) * res.toUInt();  // voltage at AN0 pin
  v /= (4.7 / 14.7);        // voltage at Vcc
  log(Log::DebugLevel::Max, QString("Vdd: %1 %2").arg(toHexLabel(res, 4)).arg(v));
  voltages[Device::VoltageType::TargetVdd] = Device::VoltageValue(Device::VoltageValue::Normal, v);
  if ( !sendCommand(0x701D, &res) ) return false;
  v = (2.050 / 256) * res.byte(0);  // voltage at AN1 pin
  v /= (10.0 / 110.0);               // voltage at Vpp
  log(Log::DebugLevel::Max, QString("Vpp: %1 %2").arg(toHexLabel(res, 4)).arg(v));
  voltages[Device::VoltageType::TargetVpp] = Device::VoltageValue(Device::VoltageValue::Normal, v);
