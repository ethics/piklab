...
void Coff::XmlToData::parse()
{
  ...
  const ::Group::Base& gpic = *Device::Lister::instance().group("pic");
  for (::Group::Base::const_iterator it = gpic.begin(); it != gpic.end(); ++it) {
    ...
    switch (data->architecture().type()) {
      ...
      case Pic::Architecture::Nb_Types: ASSERT(false); continue;
    }
  ...
  bool fatal = false;
  for (uint i = 0; pics[i].defined_as != NULL; i++) {
    _current = QString(pics[i].names[2]).upper();
    if (!Device::Lister::instance().isSupported(_current)) continue;
    if (!hasDevice(_current)) {
        qDebug(">> add new id %s: %s", _current.latin1(), toHexLabel(pics[i].coff_type, 4).latin1());
        fatal = true;
    } else {
      bool ok = false;
      ...
      if (!ok) {
          qDebug(">> different ids %s: %s", _current.latin1(), toHexLabel(pics[i].coff_type, 4).latin1());
          fatal = true;
      }
    }
  }
  if (fatal) qFatal("There were errors");
}
