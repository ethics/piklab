...
bool CDB::Object::parse()
{
  PURL::File file(_url, _log);
  ...
  for (_line=0; _line<uint(lines.count()); _line++) {
    ...
    switch (c) {
      ...
      default:
        log(Log::LineType::Error, i18n("Unrecognized record"));
        return false;
    }
    if (!ok) {
      delete record;
...
bool CDB::Object::parseLinkerRecord(Record * &record)
{
  ...
  switch (c) {
    ...
    case 'C':
      ...
      break;
    case 'X':
      lr->stype = LinkerRecord::EndAddress;
      if ( !parse(lr->scope, lr->name) ) return false;
      break;
    default:
      lr->stype = LinkerRecord::Address;
      if ( !parse(lr->scope, lr->name) ) return false;
      break;
  }
  if ( !readAndCheckChar(':') ) return false;
...
uint CDB::Object::nbRecords(Record::Type type) const
{
  uint nb = 0;
  for (uint i=0; i<uint(_records.count()); i++)
    if ( _records[i]->type()==type ) nb++;
  return nb;
}

const CDB::SymbolRecord *CDB::Object::symbol(uint i) const
{
  for (uint k=0; k<uint(_records.count()); k++) {
    if ( _records[k]->type()!=Record::Symbol ) continue;
    if ( i==0 ) return static_cast<const SymbolRecord *>(_records[k]);
    i--;
  }
  ASSERT(false);
  return 0;
