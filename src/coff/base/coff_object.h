...
END_DECLARE_ENUM_LABEL_ONLY(OptHeaderFormat)
...
END_DECLARE_ENUM_LABEL_ONLY(AuxSymbolType)
...
END_DECLARE_ENUM_LABEL_ONLY(SymbolSectionType)
...
public:
  ...
  uint section() const {
      ASSERT( sectionType()==SymbolSectionType::InsideSection );
      ASSERT( _section>0 );
      return _section - 1;
  }
...
END_DECLARE_ENUM_LABEL_ONLY(SectionType)
...
extern RegisterNameData gprNameData(const Pic::RegistersData &rdata, uint index, const Coff::Object *coff);
extern QValueVector<RegisterNameData> variableList(const Pic::RegistersData &rdata, const Coff::Object &coff);
