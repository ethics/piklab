...
QStringList SourceLine::extraConfigNames(Pic::ConfigNameType type, const Pic::Data &data, uint word, const Pic::Config::Value &value)
{
  QStringList cnames;
  QStringList names = value.extraConfigNames[type];
  if ( names.isEmpty() ) names = value.extraConfigNames[Pic::ConfigNameType::Default];
  for (uint i=0; i<uint(names.count()); i++) cnames += transformConfigName(data, word, names[i]);
  return cnames;
}
...
QStringList SourceLine::configNames(Pic::ConfigNameType type, const Pic::Memory &memory, uint word, bool &ok)
{
  ...
  for (uint k=0; k<uint(cword.masks.count()); k++) {
    ...
    for (int l=cmask.values.count()-1; l>=0; l--) {
      ...
      if ( vcnames.isEmpty() ) vcnames = cvalue.configNames[Pic::ConfigNameType::Default];
...
SourceLine::List Tool::SourceGenerator::templateSourceFile(PURL::ToolType type, const Device::Data &data, bool &ok) const
{
  ...
  std::auto_ptr<Device::Memory> memory = data.group().createMemory(data);
