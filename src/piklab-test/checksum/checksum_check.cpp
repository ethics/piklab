...
{
  Pic::Memory& memory = static_cast<Pic::Memory&>(*_memory);
  ...
  for (int i=0; i<cdata.protectedMaskNames.count(); i++) {
    ...
    for (int k=mask->values.count()-1; k>=0; k--) {
      ...
      memory.setConfigValue(pmName, mask->values[k].name);
      break;
    }
  }
  if ( !cdata.bbsize.isEmpty() ) memory.setConfigValue(protection.bootSizeMaskName(), cdata.bbsize);
}
...
void ChecksumCheck::checkChecksum(const Pic::Data &pdata, const QString &maskName, const QString &valueName, bool &ok)
{
  ...
  memory.checksumCheckFill();
  setProtection(pdata, cdata, maskName, valueName);
  if ( !checkChecksum(cdata.checkChecksum, maskName + ":" + valueName + "/" + "check") ) ok = false;
}

bool ChecksumCheck::executeDevice(const Device::Data &data)
{
  ...
  switch ( protection.family() ) {
    ...
    case Pic::Protection::Nb_Families: ASSERT(false); break;
  }
